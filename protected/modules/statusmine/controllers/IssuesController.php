...
class IssuesController extends ElementController {
	public function actionView($id, $start = 0, $count = 10) {
		...
		$issues = Redmine::readIssues('id', array(
			'sort'=>'updated_on:desc','project_id'=>$id,'status_id'=>'*','start'=>$start,'count'=>$count
		));
		parent::actionView(array(
			'issues'=>array_keys($issues),'name'=>$project['name']
		));
	}
